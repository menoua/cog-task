Fetch system once the “system” feature of “iced” becomes stable and included in main branch.

Add cross-fade support by providing fade-in/out duration (global, block, and local -- like how gain is set):
- use crossfade feature of rodio
- find similar feature on gstreamer

Add trigger support for video

Deploy progress bar at loading screen to give a sense of how long until block starts. Probably should be a progress of number of resources loaded (send message LoadProgress(done, total) on every successful load).
...
Sequence is an action that either evolves into a list of actions that run in sequence, or has an internal quasi scheduler to handle messages. SequenceTemplate will be an action that only has a src, which will evolve into Sequence. Not allowed to define it within same file, to avoid cross-contamination. Build separate graphs for each sequence. Keeps each graph simple. Maybe just reuse scheduler without separate logger. Everything else should be the same?
